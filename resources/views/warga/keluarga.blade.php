    ...
    <div class="container-keluarga">
        ...
        @if(!$user->no_kk)
            <div class="card-form"
                ...
                <h3>Pendaftaran Nomor KK</h3>
                <p>Anda belum terhubung dengan nomor KK manapun. Silahkan daftarkan nomor KK Anda.</p>
                <form action="{{ route('keluarga.store') }}" method="POST">
                    ...
                    <button type="submit" class="btn-primary"
                        style="margin-top:20px; background:var(--color-2); border:none; color:white; padding:10px 20px; border-radius:8px; cursor:pointer;">Simpan
                        Data Keluarga</button>
                </form>
            </div>
        @else
            <div class="layout-grid" style="display:grid; grid-template-columns: 1fr 2fr; gap:30px;">
                <div class="info-kk-card" style="
                    ...
                    <h4>Nomor KK:</h4>
                    <h2 style="margin-top:0;">{{ $keluarga->no_kk }}</h2>
                    <hr style="opacity:0.2;">
                    <p>Kepala Keluarga:<br><strong>{{ $keluarga->nama_kepala_keluarga }}</strong></p>

                    <div style="background:rgba(255,255,255,0.1); padding:15px; border-radius:10px; margin-top:20px;">
                        <h5 style="margin-top:0;"><i class="fas fa-user-plus"></i> Tambah Anggota</h5>
                        <form action="{{ route('keluarga.addAnggota') }}" method="POST">
                            @csrf
                            <input type="text" name="nik" placeholder="NIK Anggota" class="small-input" required>
                            <input type="text" name="nama_lengkap" placeholder="Nama Lengkap" class="small-input" required>
                ...
                <div class="table-card"
                    ...
                    <h3 style="margin-top:0;">Daftar Anggota Keluarga</h3>
                        ...
                        <tbody>
                            @foreach($keluarga->anggota as $anggota)
                                <tr style="border-bottom: 1px solid #eee;">
                                    <td style="padding:12px;">{{ $anggota->nik }}</td>
                                    <td style="padding:12px;">{{ $anggota->nama_lengkap }}</td>
                                    <td style="padding:12px;">
                                        @if($anggota->nik == Auth::user()->nik)
                                            <span style="color:var(--color-2); font-weight:bold;">(Saya)</span>
                                        @else
                                            <span style="color:#888;">Anggota</span>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
